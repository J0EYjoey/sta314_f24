...
# the default shrinkage (the parameter lambda in lecture notes) is set to 0.1
...
boot(Portfolio, alpha.fn, R = 1000)   # bootstrap with B=1000 replications
...
set.seed(1)
boot.fn(Auto,sample(n_Auto, n_Auto, replace=T))
boot.fn(Auto,sample(n_Auto, n_Auto, replace=T))

boot(Auto, boot.fn, R = 1000) # bootstrap results

# Compare the means of the Bootstrap estimates with the OLS coefficients
